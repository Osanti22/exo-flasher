...
- **Resetting the S3 native USB-JTAG needs a specific DTR/RTS order** (`pulseResetPort`). A lone RTS pulse does nothing: after `port.open()` WebSerial leaves DTR asserted, and with DTR==RTS the USB-JTAG reset cancels. Toggling DTR and RTS in the same message looks like "enter download" and lands in download mode. So: set DTR low on its own, let it settle, then pulse only RTS (EN low->high) - same as esptool.py's hard reset. The exoskeleton firmware team's own tooling notes the S3 has "no clean standalone RTS reset"; if a bench unit still won't reboot, power-cycle it.
