...
- **Two flash modes, both `eraseAll: false` and `flashMode/flashFreq/flashSize: "keep"`** (`"keep"` writes the bytes as-is, no header patching). See `buildFileArray` / `flash` in `app.js`.
  - **Update (default):** writes separate segments - `ota_data` at `0xf000`, app at `0x20000`, optional `www` at `0x830000`. It never touches `nvs` at `0x9000`, so calibration + WiFi survive. It MUST include `ota_data` (else a unit that last booted `ota_1` ignores the fresh `ota_0` app), and MUST pass separate `fileArray` entries - a merged blob spans the `nvs` gap and erases it. Offsets are the `*_OFFSET` constants; app is validated to start with `0xE9` and `ota_data` to be `<= 0x2000` (`OTADATA_MAX`).
...
- **The native USB re-enumerates on reset.** When the S3 resets into the app, the USB-Serial-JTAG drops off the bus and comes back as a fresh connection, closing the WebSerial port. The Logs monitor (`monitorLoop`) reopens the same `SerialPort` when it returns, and `onSerialDisconnect` cancels a hung read so the loop can reopen. Do not assume a port stays open across a reset.
